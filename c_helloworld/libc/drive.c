 ...
 */
void read_sector(u8 track, u8 head, u8 sector, char* buf, u8 count, u8 *status, u8 *readed)
{
	u16 code;
	__asm__ __volatile__ (
					"int $0x13\n"
					...
					"c"(track<<8 | sector), "d"(head<<8 | 0x80)
					: "memory");
	*status = (code >> 8);
	*readed = (code >> 0);
}
 ...
 */
void write_sector(u8 track, u8 head, u8 sector, char* buf, u8 count, u8 *status, u8 *wrote)
{
	u16 code;
	__asm__ __volatile__ (
					"int $0x13\n" 
